...
# --- 修改這裡：指向本機資料夾路徑 ---
# 注意：這裡是指向「資料夾」，不是指向 .safetensors 檔案本身
model_path = "./model" 
"""
您的專案資料夾/
├── script.py (您的程式碼)
...
└── model/
    ├── config.json (必須有)
    ├── preprocessor_config.json (必須有)
    ├── tokenizer.json (必須有)
    ├── tokenizer_config.json (必須有)
    └── model.safetensors (您的模型權重)
"""
# 確保使用 CUDA，如果可用
...
print(f"正在從本機載入模型：{model_path} ...")

# 從本機路徑載入 Processor (包含 tokenizer 和 image processor 設定)
processor = AutoProcessor.from_pretrained(model_path, local_files_only=True)

# 從本機路徑載入模型並移動到設備
model = AutoModelForZeroShotObjectDetection.from_pretrained(model_path, local_files_only=True).to(device)
...
if len(results) > 0:
    ...
    for score, label, box in zip(result['scores'], result['labels'], result['boxes']):
        ...
        # 繪製標籤背景 (白色背景)
        # 根據字體大小調整背景框的高度
        # 注意：textlength 需要較新版的 Pillow，舊版可用 textsize
        try:
            text_size = draw.textlength(label_text)
        except AttributeError:
            text_size = draw.textsize(label_text)[0]
